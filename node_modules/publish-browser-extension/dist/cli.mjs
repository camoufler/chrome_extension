...
async function status(config) {
	...
	if (config.dryRun) throw Error("Dry run is not supported when getting item status");
	const { chrome: resolved } = validateConfig(resolveConfig(config));
	if (!resolved) throw Error("Chrome options are required when getting item status");
	if (resolved.apiVersion !== "v2") throw Error("Only v2 API is supported when getting item status");
	const res = await new ChromeWebStoreV2(resolved, logger.success).getStatus();
	console.log();
	logger.info("Chrome");
	logger.log(`    Published item status: ${highlight(res.publishedItemRevisionStatus?.state ?? "<item not published>")}`);
	logger.log(`    Submitted item status: ${highlight(res.submittedItemRevisionStatus?.state ?? "<no submission in review>")}`);
	console.log();
}
//#endregion
//#region src/commands/set-deploy-percentage.ts
async function setDeployPercentage(config) {
	console.log();
	logger.info("Set CWS Deploy Percentage");
	if (config.dryRun) throw Error("Dry run is not supported when setting the deploy percentage");
	const { chrome: resolved } = validateConfig(resolveConfig(config));
	if (!resolved) throw Error("Chrome options are required when setting the deploy percentage");
	if (resolved.deployPercentage == null) throw Error("Deploy percentage is required when setting the deploy percentage");
	if (resolved.apiVersion !== "v2") throw Error("Only v2 API is supported when setting the deploy percentage");
	await new ChromeWebStoreV2(resolved, logger.success).setDeploymentPercentage(resolved.deployPercentage);
	console.log();
...
*/
function configFromFlagsWithFakeZip(flags, zips) {
	return configFromFlags({
		chromeZip: zips.chrome ? "..." : void 0,
		firefoxZip: zips.firefox ? "..." : void 0,
		edgeZip: zips.edge ? "..." : void 0,
		operaZip: zips.opera ? "..." : void 0,
		...flags
	});
}
...
cli.command("set-deploy-percentage", "Set the deploy percentage for the extension").action(async (flags) => {
	const config = configFromFlagsWithFakeZip(flags, { chrome: true });
	try {
...
cli.command("status", "Get the current published and submission status of the extension").action(async (flags) => {
	const config = configFromFlagsWithFakeZip(flags, { chrome: true });
	try {
		await status(config);
	} catch (err) {
